function [spectrum, rho] = compute_spectrum_according_to_threshold_lasso_logit(s, Theta, threshold, tol, max_iter, alpha, verbose)
abs_tol = 1e-6;
% initial Lambda-step for estimating an upper bound for lambda
lambda_step = 0.5;
[lambda_max, lambda_min, y_act, ~] = find_lambda_max_logit(s, Theta, lambda_step, threshold, alpha);
...
for i = 1:max_iter    
    ...
    if i == max_iter
        if rho_act > 1 - abs_tol
            ...
            [spectrum, y] = compute_spectrum_according_to_actual_spectrum_lasso_logit(spectrum_i, s, Theta, actual_lambda, alpha);
            rr = corrcoef(regenerate_signal_logit(Theta, y), s);
            rho = rr(2);  
        else
            if verbose
            ...
            if isnan(rho_act) % account for case where the last iterations yielded a NaN-value
                y_act(:) = lassoglm(Theta, s, 'binomial', 'Lambda', lambda_min, 'Alpha', alpha);
                % check whether the regenerated signal matches with the given threshold
