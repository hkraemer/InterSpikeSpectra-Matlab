function [spectrum, rho] = compute_spectrum_according_to_threshold_lasso_logit(s, Theta, threshold, tol, max_iter, verbose)
abs_tol = 1e-6;
% initial Lambda-step for estimating an upper bound for lambda
lambda_step = 0.5;
[lambda_max, lambda_min, y_act, rho_act] = find_lambda_max_logit(s, Theta, lambda_step, threshold);
...
for i = 1:max_iter    
    ...
    y = lassoglm(Theta, s, 'binomial', 'Lambda', actual_lambda);
    ...
    if i == max_iter
        if rho_act > 1 - abs_tol
            ...
            [spectrum, y] = compute_spectrum_according_to_actual_spectrum_lasso_logit(spectrum_i, s, Theta, actual_lambda);
            rr = corrcoef(regenerate_signal_logit(Theta, y), s);
            rho = rr(2);
            if verbose
